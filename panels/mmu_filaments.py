...
class Panel(ScreenPanel):
    ...
    W3C_COLORS = ['aliceblue', 'antiquewhite', 'aqua', 'aquamarine', 'azure', 'beige', 'bisque', 'black',
    ...
    def __init__(self, screen, title):
        ...
        self.use_spoolman = self._printer.spoolman and self._config.get_main_config().getboolean("mmu_use_spoolman",
                                                                                                 False)
    ...
    def select_edit(self, widget, sel_gate):
        ...
        self.labels['filament'].set_active(
            self.ui_gate_status in (self.GATE_AVAILABLE, self.GATE_AVAILABLE_FROM_BUFFER))
    ...
    def select_save(self, widget):
        ...
        self._screen._ws.klippy.gcode_script(
            f"MMU_GATE_MAP GATE={self.ui_sel_gate} "
            f"COLOR={self.ui_gate_color} MATERIAL={self.ui_gate_material} "
            f"AVAILABLE={self.ui_gate_status} SPOOLID={self.ui_gate_spool_id} "
            f"QUIET=1")
        self.labels['layers'].set_current_page(0)  # Gate list layer
