...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self.btn_states = {
            'all': ['check_gates', 'tool', 'eject', 'picker', 'pause', 'message', 'extrude', 'unlock', 'resume',
                    'manage', 'more'],
            'printing': ['pause', 'more'],
            ...
            'bypass_unloaded': ['check_gates', 'tool', 'picker', 'pause', 'message', 'extrude', 'unlock', 'resume',
                                'manage', 'more'],
            'bypass_unknown': ['check_gates', 'tool', 'eject', 'picker', 'pause', 'message', 'extrude', 'unlock',
                               'resume', 'manage', 'more'],
            'tool_loaded': ['check_gates', 'tool', 'eject', 'picker', 'pause', 'message', 'extrude', 'unlock', 'resume',
                            'manage', 'more'],
            'tool_unloaded': ['check_gates', 'tool', 'picker', 'pause', 'message', 'extrude', 'unlock', 'resume',
                              'manage', 'more'],
            'tool_unknown': ['check_gates', 'tool', 'eject', 'picker', 'pause', 'message', 'extrude', 'unlock',
                             'resume', 'manage', 'more'],
            'no_message': ['check_gates', 'tool', 'eject', 'picker', 'pause', 'extrude', 'unlock', 'resume', 'manage',
                           'more'],
            'busy': ['manage', 'more'],
            'disabled': [],
        }
        ...
        self.labels = {
            ...
            'sync_drive_img': self._gtk.Image('mmu_synced_extruder', self._gtk.img_width * 0.8,
                                              self._gtk.img_height * 0.8),  # Alternative for tool_icon
        }
        ...
        if action == "notify_status_update":
            try:
                ...
                if 'mmu' in data:
                    ...
                    if 'tool' in e_data \
                            or 'gate' in e_data \
                            or 'ttg_map' in e_data \
                            or 'gate_status' in e_data \
                            or 'gate_color' in e_data:
                        self.update_status()
                    if 'tool' in e_data \
                            or 'filament_pos' in e_data \
                            or 'filament_direction' in e_data:
                        self.update_filament_status()
                    if 'tool' in e_data \
                            or 'next_tool' in e_data \
                            or 'sync_drive' in e_data:
                        self.update_tool()
                    if 'enabled' in e_data:
                        self.update_enabled()
                    if 'action' in e_data \
                            or 'print_state' in e_data:
    ...
    def update_runout_mode(self):
        ...
        if not enabled:
            detection_mode_str = "Disabled"
        elif detection_mode == 1:
            detection_mode_str = "Clog (Man)"
        elif detection_mode == 2:
            detection_mode_str = "Clog (Auto)"
        else:
            detection_mode_str = "Clog Off"
    ...
    def update_filament_status(self):
        ...
        if self.check_toolhead_sensor() is not None:
    ...
    def get_status_text(self, markup=False):
        ...
        for g in range(num_gates):
            ...
            msg_gates += ("│#%d " % g)[:4]
            msg_avail += "│ %s " % (
                filament_not_buffered_icon if gate_status[g] == self.GATE_AVAILABLE
                else filament_icon if gate_status[g] == self.GATE_AVAILABLE_FROM_BUFFER
                else " " if gate_status[g] == self.GATE_EMPTY else "?")
            tool_str = ""
            prefix = ""
            ...
            if gate_selected == g:
                if gate_status[g] == self.GATE_AVAILABLE:
                    icon = filament_icon
                elif gate_status[g] == self.GATE_AVAILABLE_FROM_BUFFER:
                    icon = filament_icon
                elif gate_status[g] == self.GATE_EMPTY:
                    icon = " "
                else:
                    icon = "?"
                msg_selct += ("╡ %s " % icon) if g != 0 else ("│ %s " % icon)
            else:
                msg_selct += "╞═══" if gate_selected != self.GATE_UNKNOWN and gate_selected == (g - 1)\
                    else "╧═══" if g != 0 else "╘═══"
        msg_gates += "│"
        msg_tools += "│"
        msg_avail += "│"
    ...
    def get_filament_text(self, markup=False, bold=False):
        ...
        sensor_str = "│Ts│" if (self.check_toolhead_sensor() is not None and self.check_toolhead_sensor() != -1) else ""
        sensor_str_homed = "┫Ts│" if self.check_toolhead_sensor() is not None else ""
