...
class Panel(ScreenPanel):
    ...
    def add_device(self, device):
        ...
        switch = Gtk.Switch(
            hexpand=False,
            active=(self._printer.get_power_device_status(device) == "on"),
        ...
        dev = Gtk.Box(spacing=5, hexpand=True, vexpand=False, valign=Gtk.Align.CENTER)
        dev.add(labels)
        dev.add(switch)

        self.devices[device] = {"row": dev, "switch": switch}
