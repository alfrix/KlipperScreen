...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        if self.current_extruder:
            diameter = float(
                self._printer.get_config_section(self.current_extruder)["filament_diameter"]
            )
        ...
        self.buttons["button_grid"] = Gtk.Grid(
            row_homogeneous=True, column_homogeneous=True, vexpand=False
        )
    ...
    def process_update(self, action, data):
        ...
        if "print_stats" in data:
            ...
            if "info" in data["print_stats"]:
                if (
                    "total_layer" in data["print_stats"]["info"]
    ...
    def estimate_time(
        self, progress, print_duration, file_time, filament_time, slicer_time, last_time
    ):
        estimate_above = 0.3
        ...
        weight_filament = (
            min(progress - estimate_above, 0.33) if print_duration < filament_time else 0
        )
    ...
    def _update_file_metadata(self):
        ...
        if "estimated_time" in self.file_metadata:
            ...
            self.labels["slicer_time"].set_label(
                self.format_time(self.file_metadata["estimated_time"])
            )
