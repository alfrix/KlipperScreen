...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self.btn_states = {
            'all': ['gate', 'checkgate', 'recover', 'load', 'eject', 'home', 'motors_off', 'servo_up', 'servo_move',
                    'servo_down', 'load_ext', 'unload_ext'],
            'not_homed': ['recover', 'eject', 'home', 'motors_off', 'servo_up', 'servo_move', 'servo_down', 'load_ext',
                          'unload_ext'],
            'servo_up': ['gate', 'checkgate', 'recover', 'load', 'eject', 'home', 'motors_off', 'servo_move',
                         'servo_down', 'load_ext', 'unload_ext'],
            'servo_move': ['gate', 'checkgate', 'recover', 'load', 'eject', 'home', 'motors_off', 'servo_up',
                           'servo_down', 'load_ext', 'unload_ext'],
            'servo_down': ['gate', 'checkgate', 'recover', 'load', 'eject', 'home', 'motors_off', 'servo_up',
                           'servo_move', 'load_ext', 'unload_ext'],
            'bypass_loaded': ['recover', 'eject', 'motors_off', 'servo_up', 'servo_move', 'servo_down', 'unload_ext'],
            'bypass_unloaded': ['gate', 'checkgate', 'recover', 'load', 'home', 'motors_off', 'servo_up', 'servo_move',
                                'servo_down', 'load_ext', 'unload_ext'],
            'bypass_unknown': ['gate', 'checkgate', 'recover', 'load', 'eject', 'home', 'motors_off', 'servo_up',
                               'servo_move', 'servo_down', 'load_ext', 'unload_ext'],
            'tool_loaded': ['recover', 'eject', 'motors_off', 'servo_up', 'servo_move', 'servo_down', 'unload_ext'],
            'tool_unloaded': ['gate', 'checkgate', 'recover', 'load', 'home', 'motors_off', 'servo_up', 'servo_move',
                              'servo_down', 'load_ext', 'unload_ext'],
            'tool_unknown': ['gate', 'checkgate', 'recover', 'load', 'eject', 'home', 'motors_off', 'servo_up',
                             'servo_move', 'servo_down', 'load_ext', 'unload_ext'],
            'busy': [],
            'disabled': [],
        }
        ...
        self.labels['recover'].connect("clicked", self.menu_item_clicked,
                ...
                if 'action' in e_data:
                    ...
                    if self.ui_action_button_name is not None:
                        if action == "Idle" or action == "Unknown":
                            self.labels[self.ui_action_button_name].set_label(
                                self.ui_action_button_label)  # Restore original button label
                            self.ui_action_button_name = None
                        else:
                            self.labels[self.ui_action_button_name].set_label(
                                action)  # Use button to convey action status
    ...
    def select_load(self, widget):
        ...
        self._screen._ws.klippy.gcode_script(
            f"MMU_LOAD TEST=0")  # TEST=0 is to aid backward compatibility of MMU_LOAD command
    ...
    def update_active_buttons(self):
        ...
        if enabled:
            if servo == "Up":
                ui_state.append("servo_up")
            elif servo == "Down":
                ui_state.append("servo_down")
            elif servo == "Move":
                ui_state.append("servo_move")
            else:
                ui_state.append("all")
            if not is_homed:
