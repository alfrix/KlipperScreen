...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        for i in range(num_gates):
            ...
            available = self.labels[f'available_{i}'] = Gtk.Label(label="Unknown")
            ...
            gate_label = self.labels[f'gate_label_{i}'] = Gtk.Label(label=f'#{i}')
            gate_label.get_style_context().add_class("mmu_spoolman_gate_text")
            gate_box.pack_start(gate_label, True, True, 0)

            color = self.labels[f'color_{i}'] = Gtk.Label(label=f'⬤')
    ...
    def refresh(self):
        ...
        for i in range(num_gates):
            ...
            for a in [self.labels[f'gate_label_{i}'], self.labels[f'available_{i}'], self.labels[f'tools_{i}'],
                      self.labels[f'filament_{i}'], self.labels[f'remaining_percentage_{i}'],
                      self.labels[f'remaining_weight_{i}'], self.labels[f'material_{i}'], self.labels[f'color_{i}']]:
            ...
            if str(gate_spool_id[i]) in self.spools:
                ...
                self.labels[f'color_image_{i}'].set_from_pixbuf(
                    pixbuf.scale_simple(pixbuf.get_width() * 0.9, pixbuf.get_height() * 0.9, 1))
    ...
    def process_update(self, action, data):
        if action == "notify_status_update":
            if 'configfile' in data:
                ...
                e_data = data['mmu']
                if 'ttg_map' in e_data \
                        or 'gate' in e_data \
                        or 'gate_status' in e_data \
                        or 'gate_spool_id' in e_data:
                    self.refresh()
